...
#运行成功
I'll scan the current `Cargo.toml` and key source files to integrate utoipa and Swagger UI for API docs, then add the necessary mounts and derives.

[5 tools called]

I'll add the Swagger UI mount to serve the generated OpenAPI at `/docs`, wiring it to `routes::ApiDoc::openapi()`, then check for linter issues.

[2 tools called]

- I’m going to serve the API docs using utoipa + Swagger UI by mounting a docs route.

我已将 OpenAPI 文档挂载到项目：
- 生成文档：`routes::ApiDoc::openapi()`（你已有 `#[derive(OpenApi)]` 配置）
- 文档 UI：已在 `src/main.rs` 增加 Swagger UI 挂载，访问路径：
  - 文档页面：`/docs`
...
如果你还想同时启用 RapiDoc、Redoc 或 Scalar（和参考文档一样多样化），告诉我，我可以一并加上对应挂载。